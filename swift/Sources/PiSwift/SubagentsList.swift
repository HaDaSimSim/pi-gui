...
struct SubagentsList: View {
    ...
    var body: some View {
        if runs.isEmpty {
            ...
            .sheet(item: Binding(get: { selected.map { RunBox($0) } }, set: { if $0 == nil { selected = nil } })) { box in
        ...
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 5) {
                    ...
                    if run.turns.count > 1 { Text("\(run.turns.count) turns").font(.caption2).foregroundStyle(.tertiary) }
                    if run.cost > 0 { Text(Fmt.cost(run.cost)).font(.caption2).foregroundStyle(.tertiary) }
                    Text(run.stale ? "stale" : run.status).font(.caption2).foregroundStyle(run.stale ? Theme.danger : Color.secondary)
                }
            }
            Spacer(minLength: 0)
            Image(systemName: "chevron.right").font(.system(size: 10)).foregroundStyle(.tertiary).padding(.top, 3)
        }
        .padding(8)
