...
final class ProviderStore: ObservableObject {
    ...
    func load() {
        guard let data = FileManager.default.contents(atPath: path) else { raw = ["providers": [String: Any]()]; return }
        if let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            raw = obj; loadError = nil
        } else {
    ...
    var providers: [ProviderView] {
        ...
        return provs.keys.sorted().compactMap { name in
            ...
            let models = (p["models"] as? [[String: Any]] ?? []).map { m in
                ModelView(modelId: (m["id"] as? String) ?? "",
                          name: (m["name"] as? String) ?? (m["id"] as? String) ?? "",
                          contextWindow: (m["contextWindow"] as? Int) ?? 0)
            }
            return ProviderView(name: name,
