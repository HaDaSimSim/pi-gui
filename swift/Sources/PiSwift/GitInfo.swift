...
struct GitInfo {
    ...
    static func load(cwd: String, logLimit: Int = 50) -> GitInfo {
        var info = GitInfo()
        guard run(["rev-parse", "--is-inside-work-tree"], cwd: cwd)
            .trimmingCharacters(in: .whitespacesAndNewlines) == "true" else { return info }
        info.isRepo = true
        ...
        for line in run(["status", "--porcelain"], cwd: cwd).split(separator: "\n") {
            ...
            if x == "?" && y == "?" { info.untracked.append(GitFileChange(path: path, code: "?")); continue }
        ...
        let logOut = run(["log", "-\(logLimit)", "--pretty=format:\(logFmt)", "--all", "--date-order"], cwd: cwd)
        for line in logOut.split(separator: "\n") {
            let f = line.components(separatedBy: us)
            guard f.count >= 7 else { continue }
            let refs = f[5].split(separator: ",").map { $0.trimmingCharacters(in: .whitespaces) }.filter { !$0.isEmpty }
            let parents = f[6].split(separator: " ").map(String.init)
            info.commits.append(GitCommit(hash: f[0], shortHash: f[1], subject: f[2],
                                          author: f[3], relTime: f[4], refs: refs, parents: parents))
        }

        // Branches.
        let brFmt = ["%(refname:short)", "%(HEAD)", "%(upstream:short)"].joined(separator: us)
        for line in run(["for-each-ref", "--format=\(brFmt)", "refs/heads"], cwd: cwd).split(separator: "\n") {
            let f = line.components(separatedBy: us)
            guard f.count >= 2 else { continue }
            info.branches.append(GitBranchInfo(name: f[0], current: f[1] == "*",
    ...
    static func commitDetail(cwd: String, hash: String) -> GitCommitDetail? {
        guard hash.range(of: "^[0-9a-fA-F]{4,40}$", options: .regularExpression) != nil else { return nil }
        let us = "\u{1f}", rs = "\u{1e}"
        ...
        for line in out[sep.upperBound...].split(separator: "\n") {
            ...
            let a = Int(parts[0]) ?? 0, d = Int(parts[1]) ?? 0
            ins += a; del += d
            files.append(GitCommitFile(path: parts[2], added: a, deleted: d))
        }
        return GitCommitDetail(hash: meta[0], shortHash: meta[1], subject: meta[2], body: meta[3],
