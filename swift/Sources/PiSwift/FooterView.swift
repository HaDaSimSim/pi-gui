    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            ...
            HStack(spacing: 6) {
                ...
                if let name = runtime.sessionName { Text("•"); Text(name) }
                Spacer()
            }
            // goal + todos
            ...
            HStack(spacing: 8) {
                ...
                Text("↑\(Fmt.tokens(f.inputTokens)) ↓\(Fmt.tokens(f.outputTokens)) R\(Fmt.tokens(f.cacheRead)) W\(Fmt.tokens(f.cacheWrite)) \(Fmt.cost(f.cost))")
            ...
            // ownership
            HStack { Text(lockText).foregroundStyle(lockColor); Spacer() }
        }
