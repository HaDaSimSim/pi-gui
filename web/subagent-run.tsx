// Subagent run view — rendered in the info panel's "Subagents" tab.
// Reads and displays the subagent-run custom entries the subagents extension left in the session file.
    ...
    <div className="rounded-lg border p-3">
      <Collapsible open={open} onOpenChange={setOpen}>
        ...
        <CollapsibleContent className="mt-2.5 flex flex-col gap-2.5">
          ...
          {run.turns.map((tn, i) => (
            <div key={`${run.runId}-${tn.prompt}`} className="border-l-2 border-border pl-2.5">
              {run.turns.length > 1 ? (
