...
enum SelfTest {
  ...
  static func testRpc() {
    ...
    Task { @MainActor in
      let rt = RuntimeSession(
        cwd: NSTemporaryDirectory(), piPath: config.piPath,
        model: config.defaultModelSpec, sessionDir: sessionDir)
      do { try rt.start() } catch {
      ...
      rt.sendPromptWhenReady("Reply with exactly: PONG") {
        ...
        for it in rt.items {
          if case .assistant(_, let am) = it {
            print("  assistant: \(am.text.prefix(80))")
            if am.text.contains("PONG") { sawText = true }
          }
        }
        print(
          "  footer cost=\(String(format: "%.4f", rt.footer.cost)) sessionPath=\(rt.sessionPathForTest ?? "nil")"
        )
        rt.dispose()
        done.signal()
      }
  ...
  static func testLock() {
    ...
    let a = SessionLock(
      sessionPath: sessionPath, owner: "pi-web", label: "pi-gui: test", lockDir: tmp)
    let r1 = a.tryAcquire()
      ...
      let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
    {
      ...
      assert(
        keys == ["sessionPath", "owner", "pid", "host", "label", "since", "token"],
    ...
    b.takeover()
    print(
      "  after b.takeover: a.isMine=\(a.isMine()) a.isLost=\(a.isLost()) b.isMine=\(b.isMine())")
  ...
  static func testResume() {
    ...
    var ok = false
    Task { @MainActor in
      let rt1 = RuntimeSession(
        cwd: NSTemporaryDirectory(), piPath: config.piPath,
        model: config.defaultModelSpec, sessionDir: dir)
      do { try rt1.start() } catch {
        print("  start1 failed: \(error)")
        done.signal()
        return
      }
  ...
  static func testParser() {
    ...
    for it in items {
      ...
      case .subagentRun(_, let run):
        subRuns += 1
        if !run.turns.isEmpty { subWithTurns += 1 }
      default: break
      }
    }
    ...
    print(
      "  footer: in=\(footer.inputTokens) out=\(footer.outputTokens) cost=\(String(format: "%.4f", footer.cost)) model=\(footer.model ?? "nil") todos=\(footer.todosDone)/\(footer.todosTotal)"
    )
    ...
    if let big = sessions.max(by: { $0.sizeBytes < $1.sizeBytes }) {
      ...
      print(
        "  LARGEST (\(big.sizeBytes / 1024)KB): tail \(be.count) entries in \(String(format: "%.3f", Date().timeIntervalSince(t1)))s"
      )
    }
    print("  OK")
  }
