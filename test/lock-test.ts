// Lock protocol verification. Simulates two holders (A=pi, B=pi-web) within one process.
// In reality tokens differ per process/instance, so this models it with two SessionLock instances.

import { mkdtempSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { listLocks, SessionLock } from '../shared/session-lock.ts';
...
console.log('5) 핵심: 보내기 직전 A가 확인하면 차단되어야 함');
{
  ...
  // → the extension/runtime cancels/disposes at this point
}

console.log('6) lost 된 A가 release 해도 B 락은 안 건드림');
{
  A.release(); // A doesn't own it, so the file isn't deleted
