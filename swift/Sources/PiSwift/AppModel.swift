...
final class AppModel: ObservableObject {
  ...
  func openSession(_ summary: SessionSummary) {
    ...
    let tab = Tab(
      title: summary.name ?? summary.preview.map { String($0.prefix(40)) } ?? "Untitled session",
  ...
  /// Cmd+1…9: activate the Nth tab (9 = last tab, matching browsers).
  func activateTab(number n: Int) {
    guard !tabs.isEmpty else { return }
    let idx = (n == 9) ? tabs.count - 1 : min(n - 1, tabs.count - 1)
    activeTabID = tabs[idx].id
  }

  /// Ctrl-Tab / Ctrl-Shift-Tab: cycle the active tab, wrapping around.
  func cycleTab(by delta: Int) {
    guard !tabs.isEmpty else { return }
    let cur = tabs.firstIndex { $0.id == activeTabID } ?? 0
    let next = ((cur + delta) % tabs.count + tabs.count) % tabs.count
    activeTabID = tabs[next].id
  }

  /// Tear down every runtime (releases each session lock + terminates pi). Called on app quit
  /// so Cmd-Q doesn't leave orphan locks / lingering pi processes.
  func disposeAll() {
    for tab in tabs { tab.runtime.dispose() }
  }
