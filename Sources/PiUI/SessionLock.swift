...
public struct LockRecord: Codable, Equatable {
  public var sessionPath: String
  public var owner: String  // "pi" | "pi-web"
  public var pid: Int
  public var host: String
  public var label: String?
  public var since: Double  // epoch ms (JS Date.now())
...
public func defaultLockDir() -> String {
...
private func newToken() -> String {
  ...
  let rand = String(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(6))
    .lowercased()
  return "\(pid)-\(now)-\(rand)"
}
...
public func isStaleRecord(_ rec: LockRecord) -> Bool {
  let host = hostName()
  // Case-insensitive host compare: Node os.hostname() and Swift ProcessInfo.hostName can
  // differ only in casing (e.g. "Foo.local" vs "foo.local") on the same machine. A
  // case-sensitive compare here would make the TUI and pi-gui treat each other's locks as
  // "different host" and never reap orphans — breaking same-machine interop.
  if !rec.host.isEmpty && !host.isEmpty
    && rec.host.caseInsensitiveCompare(host) != .orderedSame
  {
    return false
  }
...
public final class SessionLock {
  ...
  public init(
    sessionPath: String, owner: String, label: String? = nil, lockDir: String = defaultLockDir()
  ) {
  ...
  public static func fileName(for sessionPath: String) -> String { "\(keyFor(sessionPath)).json" }
  ...
  @discardableResult
  public func tryAcquire() -> (acquired: Bool, current: LockRecord?) {
    let rec = read()
  ...
  @discardableResult
  public func takeover() -> LockRecord? {
    let prev = read()
    let token = newToken()
    myToken = token
    write(
  ...
  public func isMine() -> Bool {
    if case .mine = state() { return true }
    return false
  }
...
public func listLocks(lockDir: String = defaultLockDir()) -> [LockRecord] {
