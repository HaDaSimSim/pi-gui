// SSE 라이브 스트리밍 E2E.
// 실제 모델(relay)로 프롬프트를 던지고, SSE 로 text_delta 가 흐르는지,
// 락+런타임이 실제 토큰 흐름에서 맞물리는지 끝까지 확인한다.
...
try {
  ...
  const sseDone = (async () => {
    ...
    ok(res.headers.get("content-type")?.includes("text/event-stream") ?? false, "content-type event-stream");
    ...
    try {
      while (true) {
        ...
        while ((idx = buf.indexOf("\n\n")) !== -1) {
          ...
          if (ev.type === "agent_end") { sawAgentEnd = true; ac.abort(); return; }
        }
      }
    } catch { /* aborted */ }
  })();
  ...
  // 토큰이 흐르고 agent_end 까지 기다린다 (최대 30s)
  await Promise.race([
    sseDone,
  ...
  ok(events.some((e) => e.type === "_connected"), "_connected 이벤트 수신");
  ok(events.some((e) => e.type === "agent_start"), "agent_start 수신");
