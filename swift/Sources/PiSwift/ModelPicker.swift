...
struct ModelPicker: View {
    ...
    private var filtered: [ModelOption] {
        ...
        return model.models.filter { $0.spec.localizedCaseInsensitiveContains(query) || $0.name.localizedCaseInsensitiveContains(query) }
    }
    ...
    var body: some View {
        Menu {
            ForEach(levels, id: \.self) { lvl in
                Button { runtime.setThinking(lvl) } label: {
                    HStack {
                        Text(lvl)
