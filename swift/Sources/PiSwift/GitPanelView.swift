...
struct GitPanelView: View {
    ...
    var body: some View {
        ...
        .sheet(item: Binding(get: { detail.map { CommitDetailBox($0) } }, set: { if $0 == nil { detail = nil } })) { box in
    ...
    private func reload() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async { info = fresh; loaded = true; loading = false }
        }
    }
}
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                HStack(spacing: 5) {
                    Text(commit.shortHash).font(.system(.caption2, design: .monospaced)).foregroundStyle(.tertiary)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        if !detail.parents.isEmpty {
                            Text("parents: \(detail.parents.map { String($0.prefix(8)) }.joined(separator: ", "))")
                    ...
                    ForEach(detail.files) { f in
                        HStack(spacing: 8) {
                            Text("+\(f.added)").font(.system(.caption2, design: .monospaced)).foregroundStyle(Theme.success).frame(width: 38, alignment: .trailing)
                            Text("−\(f.deleted)").font(.system(.caption2, design: .monospaced)).foregroundStyle(Theme.danger).frame(width: 38, alignment: .trailing)
